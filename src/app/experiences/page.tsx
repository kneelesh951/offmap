  ...
  return (
    <>
      ...
      <main>
        ...
        {/* Category pills */}
        <div className="px-5 md:px-11 py-6 bg-white border-b" style={{ borderColor: 'rgba(15,61,34,0.10)' }}>
          <div className="max-w-6xl mx-auto flex gap-2 overflow-x-auto">
            {CATEGORIES.map(c => (
              <button key={c} className="flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium border transition-all"
                style={{ borderColor: 'rgba(15,61,34,0.15)', backgroundColor: c === 'All' ? GREEN : 'transparent', color: c === 'All' ? '#fff' : GREEN }}>
                {c}
              </button>
            ))}
          </div>
          ...
          <div className="max-w-6xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {EXPERIENCES.map(e => (
              <div key={e.title} className="rounded-2xl overflow-hidden border group cursor-pointer hover:-translate-y-1 transition-all duration-300"
                style={{ borderColor: 'rgba(15,61,34,0.10)', boxShadow: '0 2px 8px rgba(15,61,34,0.08)' }}>
                {/* Cover */}
                ...
                <div className="p-5">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full" style={{ backgroundColor: '#EAF5EE', color: GREEN }}>{e.category}</span>
                  ...
                  <p className="text-xs mb-3" style={{ color: '#4A7A5C' }}>With {e.host}</p>
                  <p className="text-sm leading-relaxed mb-4 line-clamp-3" style={{ color: '#3D8055' }}>{e.desc}</p>
                  <div className="flex items-center justify-between pt-3 border-t" style={{ borderColor: 'rgba(15,61,34,0.10)' }}>
                    <div>
