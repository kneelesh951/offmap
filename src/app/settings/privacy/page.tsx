...
export default async function PrivacySettingsPage() {
  ...
  if (isMock) {
    ...
    if (!user) redirect('/auth/login?redirect=/settings/privacy')
    ...
    if (!authUser) redirect('/auth/login?redirect=/settings/privacy')
    ...
    try {
      ...
      if (!dbUser) redirect('/auth/login?redirect=/settings/privacy')
      ...
      redirect('/auth/login?redirect=/settings/privacy')
    }
  }

  if (!sessionUser) redirect('/auth/login?redirect=/settings/privacy')
      ...
      <main className="min-h-screen pt-[68px]" style={{ backgroundColor: '#F5F2EC' }}>
        ...
        <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg,#0C3520 0%,#084E4E 60%,#0a5e5e 100%)' }}>
          ...
          <div className="relative max-w-5xl mx-auto px-5 md:px-11 py-10">
            ...
            <h1 className="font-serif text-4xl font-bold text-white mb-1" style={{ letterSpacing: '-0.03em' }}>Privacy & data</h1>
